 ...
 * atributos estáticos,
 * proporcionando un punto de acceso global a la colección de eventos y al
 * evento activo.
 * 
 * Responsabilidades:
 ...
 * - iFormularioAprendiz/iFormularioInvitado llaman a getEventoActivo() para
 * validar código
 * - iAdminEvento usa getEventoActivo() para preseleccionar eventos en el
 * ComboBox
 * 
 ...
 * 
 * Patrón de diseño: Repository Pattern + Singleton (implícito) + State Pattern
 * (evento activo)
 * 
...
public class GestorEventos {
  ...
  /**
   * Variable estática que almacena el evento actualmente seleccionado/activo en
   * el sistema.
   * 
   * Características:
   * - **static**: Compartida globalmente, accesible desde cualquier parte del
   * sistema
   ...
   * - Esta lista NO es final, su referencia puede cambiar (aunque no se hace en
   * la práctica)
   ...
   *               los datos del evento: nombre, fecha/hora, lugar y código de
   *               asistencia
   */
   ...
   * 
   * - **Formularios de registro**: Obtener el evento al cual el asistente se está
   * registrando
   * - **Validación de código**: Comparar el código ingresado con
   * evento.getCodigoAsistencia()
   * - **Preselección**: Mostrar el evento activo preseleccionado en ComboBox de
   * iAdminEvento
   * 
   * Ejemplo de uso:
   * ```java
   ...
   * 
   * @return Objeto Evento que representa el evento activo, o null si no hay
   *         ningún
   ...
   * - **iAdminEvento**: Para poblar el ComboBox de filtrado de registros por
   * evento
   ...
   * - GestorRegistros retorna la referencia directa (menos seguro pero más
   * eficiente)
   ...
   *         La lista original no se ve afectada por modificaciones a la lista
   *         retornada.
   */
