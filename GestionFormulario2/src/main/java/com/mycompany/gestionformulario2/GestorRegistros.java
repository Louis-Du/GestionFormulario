...
/**
 * Clase gestora (Manager/Repository) que administra todos los registros de asistencia a eventos.
 * 
 ...
 * 
 * Uso en el sistema:
 * - iFormularioAprendiz e iFormularioInvitado llaman a agregarRegistro() al confirmar un registro
 * - iAdminEvento llama a obtenerRegistros() para mostrar todos los registros en la tabla
...
public class GestorRegistros {
     ...
     * 
     * Características:
     * - **static**: Compartida por todas las instancias (aunque la clase no se instancia)
     ...
     * - ArrayList de java.util es una implementación de List basada en arreglos dinámicos
     ...
     * 
     * Este método es llamado desde los formularios de registro (iFormularioAprendiz,
     ...
     * 5. El registro se agrega a la lista estática y queda disponible para consultas
     * 
     * Nota: Este método no valida duplicados ni datos nulos. La validación debe
     * realizarse antes de llamar a este método.
     * 
     * @param registro Objeto Registro (de la clase Registro.java) que contiene todos
     *                 los datos del asistente: nombre, apellidos, documento, programa,
     ...
     * 
     * Este método proporciona acceso de lectura a todos los registros y es utilizado por:
     * - **iAdminEvento**: Para mostrar todos los registros en la tabla de asistentes
     * - **iAdminEvento (filtrado)**: Para filtrar registros por evento específico
     * - **ExportadorExcel**: Para obtener los registros a exportar (después de filtrar)
     * 
     ...
     * Para mayor seguridad, se podría retornar una copia: new ArrayList<>(registros)
     * 
     ...
     *         La lista está ordenada por orden de inserción (FIFO - First In First Out)
     */
