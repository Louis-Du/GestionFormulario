...
/**
 * Clase modelo que representa un evento dentro del sistema de gestión de
 * formularios.
 * 
 ...
 * 
 * Uso en el sistema:
 * - Objetos de esta clase son creados en iAdminEvento mediante el diálogo
 * "Crear Evento"
 ...
 * - El código de asistencia es validado en iFormularioAprendiz e
 * iFormularioInvitado
 * 
...
public class Evento {
   ...
   * Validación: Mínimo 5 caracteres, no puede estar vacío (ver
   * iAdminEvento.esNombreValido())
   ...
   * Validación: Solo letras, números y espacios (ver
   * iAdminEvento.esLugarValido())
   ...
   * Los usuarios deben ingresar este código al registrarse para confirmar su
   * asistencia.
   * Validación: Alfanumérico sin espacios, mínimo 4 caracteres (ver
   * iAdminEvento.esCodigoValido())
   ...
   * 4. El evento se registra en GestorEventos mediante
   * GestorEventos.registrarEvento()
   * 
   * @param nombre           Nombre descriptivo del evento (validado, mínimo 5
   *                         caracteres)
   * @param fechaHora        Fecha y hora programada del evento (obtenida de
   *                         JDateChooser + JSpinner)
   * @param lugar            Lugar donde se realizará el evento (validado,
   *                         alfanumérico con espacios)
   * @param codigoAsistencia Código único para validar la asistencia (validado,
  ...
  // mayúscula)
  // NO hay setters (mutadores) porque los eventos son inmutables después de
  // creados
  // Son usados por:
