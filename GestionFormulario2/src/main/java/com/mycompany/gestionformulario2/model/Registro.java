 ...
 * 
 * Esta clase es una estructura de datos (POJO - Plain Old Java Object) que
 * encapsula
 ...
 * 
 * Uso en el sistema:
 * - Objetos de esta clase son creados en iFormularioAprendiz e
 * iFormularioInvitado
...
public class Registro {
   ...
   * Define si el asistente es un aprendiz de la institución o un visitante
   * externo
   */
  ...
  /**
   * Constructor principal que inicializa un nuevo registro con todos los datos
   * del asistente.
   * 
   * Este constructor es utilizado por las clases de formulario
   * (iFormularioAprendiz,
   * iFormularioInvitado) para crear un nuevo objeto Registro con los datos
   * ingresados
   * por el usuario.
   * 
   * Flujo de uso:
   ...
   * 4. El registro se agrega a GestorRegistros mediante
   ...
   * @param apellidos         Apellidos del asistente (validado, mínimo 3
   *                          caracteres)
   * @param tipoDocumento     Tipo de documento de identificación (CC, TI, CE,
   *                          Pasaporte)
   ...
   * @param celular           Número de teléfono celular (validado, mínimo 10
   *                          dígitos)
   * @param correo            Dirección de correo electrónico (validado con
   *                          formato email)
   * @param nombreEvento      Nombre del evento al que se registra (obtenido de
   *                          Evento)
   * @param fechaHoraRegistro Fecha y hora del registro de asistencia (generado
   *                          con SimpleDateFormat)
   * @param estado            Estado actual de la asistencia ("Presente",
   *                          "Ausente", etc.)
   */
  ...
  // Siguen el patrón JavaBean: get + NombreAtributo (con primera letra en
  // mayúscula)
  // Son usados por:
   ...
   * Este valor proviene de Evento.getNombre() o
   * GestorEventos.getEventoActivo().getNombre()
   * 
   ...
   * 
   * @param recordatorioEnviado true para marcar como enviado, false en caso
   *                            contrario
   */
