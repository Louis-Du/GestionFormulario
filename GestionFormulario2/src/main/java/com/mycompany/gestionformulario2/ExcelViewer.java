...
/**
 * Clase utilitaria para cargar y visualizar archivos Excel en componentes JTable de Swing.
 * Utiliza la librería Apache POI para leer archivos Excel (.xlsx).
 */
public class ExcelViewer {
    /**
     * Método estático que carga un archivo Excel desde un InputStream y lo convierte en una JTable.
    ...
    private static Object obtenerValor(Cell cell) {
        ...
        switch (cell.getCellType()) {
            case STRING: return cell.getStringCellValue();   // Texto
            case NUMERIC: return cell.getNumericCellValue(); // Números (enteros o decimales)
            case BOOLEAN: return cell.getBooleanCellValue(); // Valores booleanos (true/false)
            case FORMULA: return cell.getCellFormula();      // Fórmulas de Excel
            default: return "";                              // Cualquier otro tipo retorna cadena vacía
        }
    }
}
